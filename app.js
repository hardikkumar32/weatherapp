...
let callingapi = async(value)=>{
    const call = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${value}&units=metric&appid=f43b7e627ff058e78bad68302373fcd2`)
    // console.dir(call.data);
    document.body.style.backgroundImage = `url(https://source.unsplash.com/1920x1080/?${value})`;
    city.innerHTML = call.data.name;
    document.querySelector(".icon").src = `https://openweathermap.org/img/wn/${call.data.weather[0].icon}.png`
    otherweather(call);
